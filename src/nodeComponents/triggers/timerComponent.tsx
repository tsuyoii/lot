...
export const FakeData: React.FC<Params> = (props) => {
    ...
    const transToSec = (timer:number,unitType:string) => {
        return Number(Time[unitType])*timer
    }
    const secToTime = (sec:number | string) => {
        const unitType = Number(sec) >= 24*3600 ? 'day' :Number(sec) >= 3600?'hour':Number(sec) >= 60?'minutes':'seconds'
        const timer = Number(sec) / Time[unitType]
        return {timer,unitType}
    }

    const selectAfter = (
        <Select defaultValue={unit[0]} style={{width: 100}} value={unitType} onChange={handleUnitChange}>
            {unit.map(type => (
    ...
    return (
        <Space direction="vertical">
            ...
            <Row>
                <Select defaultValue={scheduleData[0].value} value={schedule} onChange={handleScheduleChange} style={{width:'100%'}}>
            ...
            <Row align="middle">
                <Col span={2}>每</Col>
                <Col span={19}>
                <InputNumber addonAfter={selectAfter} defaultValue={1} value={timer} onChange={handleTimerChange}/>
                </Col>
                <Col span={3}>一次</Col>
            </Row>
        </Space>
